@description('''Required. The id of the resource to apply role assignments to.
This resource **must** be in the resourceGroup you are deploying against, unless the deployment targets the subscription or management group level.''')
param resourceId string

@description('Required. The name of the role. Must be a built-in or a custom role defined at the management group level.')
@allowed([
  ...
  'Microsoft.DevCenter Deployment Environments User'
  'Microsoft.Kubernetes connected cluster role'
  'Monitoring Alert Contributor'
...
var builtInRoles = json(loadTextContent('roles.jsonc'))
...
module resourceRoleAssignment 'resourceRoleAssignment.jsonc' = [for (principalId, index) in principalIds: {
  name: '${deploymentName}_rbac${index}'
  scope: resourceGroup(subscriptionId, resourceGroupName)
  params: {
    resourceId: resourceId
    principalId: principalId
    roleDefinitionId: '/subscriptions/${subscriptionId}/providers/Microsoft.Authorization/roleDefinitions/${builtInRoles[roleName]}'
  }
}]

@description('Resource Ids of the Resource Roles assigned')
output roleAssignmentIds array = [for (principalId, index) in principalIds: resourceRoleAssignment[index].outputs.id]
